...
if mesh == 1:
    particles = int(1.5e6)  # Number of particles per batch for meshed model (increase if rel stdev too large)
else:
    particles = int(1.5e4)  # Number of particles per batch for non-meshed model

# Lithium Oxide pellet bed properties
pellet_porosity = 0.20  # Guess of 20% porosity for Li20 pellets
packing_efficiency = (
...
if __name__ == "__main__":
    ...
    for file in glob.glob(os.path.join(results_folder, "Li2O_pellets_results.h5")):
        os.remove(file)
    ...
    processed_data_file = os.path.join(results_folder, "Li2O_pellet_results.json")
    ...
    new_sp_path = os.path.join(results_folder, "Li2O_pellet_results.h5")
    ...
    existing_data.update(Li2O_pellet_results)
