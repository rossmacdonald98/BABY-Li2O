# -----------------------------------------------------------------------------
# Tritium Transport in Sintered Lithium Ceramic Pellets Simulation, with Packed Bed (Plug Flow Model)
#
# This script simulates the generation, transport, trapping, and release of
# tritium in a packed bed of a sintered lithium ceramic pellet breeder material,
...
# 2. Initialize concentration arrays and other variables.
# 3. Run the main simulation loop, updating concentrations and calculating derived quantities.
# 4. Store results and export to a JSON file.
# -----------------------------------------------------------------------------
...
fr = (
    7.716 * porosity_pellet**2
)  # Surface Area Reduction Factor (accounts for necking due to sintering between grains)
...
z = np.linspace(
    dz / 2, z_bed - dz / 2, Nz
)  # Axial positions of each node (center of each plug)
...
Sv0 = (
    0.64 / V_grain
) * A_grain  # Specific surface area of unsintered grains, assuming random close packing of spheres (0.64) (cm^-1)
Sv = Sv0 * (porosity_pellet / 0.36) ** (
    2 / 3
)  # Specific surface area of sintered pellets (cm^-1)
...
N_grains_pellet = (
    V_pellet * (1 - porosity_pellet) / V_grain
)  # Number of grains per pellet
...
N_pellets_plug = (V_plug * packing_density) / (
    V_pellet
)  # Number of Pellets in one plug
A_pellets_plug = (
    N_pellets_plug * A_external
)  # Total External Surface Area of Pellets in one plug (cm^2)


# Tritium production
V_breeding_total = V_bed * packing_density * (1 - porosity_pellet)
T_est = (
    G_rate * V_breeding_total * t_irr * decay_constant
)  # Estimated total tritium produced (Bq)
...
key_times = [0, t_irr / 4, t_irr / 2, 3 * t_irr / 4, t_irr]
# Five equally spaced times after irradiation
post_irr_times = np.linspace(t_irr, total_sim_time, 6)[
    1:
]  # skip t_irr, already included
...
while current_time < total_sim_time:
    ...
    for j in range(Nz):
        ...
        Cm[j, 0] = (
            Cm_old[j, 0]
            + (
                dCm_dt_diffusion_center
                + G
                - dCm_dt_trapping_center
                + dCm_dt_detrapping_center
            )
            * dt
        )
        Ct[j, 0] = (
            Ct_old[j, 0] + (dCm_dt_trapping_center - dCm_dt_detrapping_center) * dt
        )

        # Interior Nodes (1 to N-1)
        for i in range(1, N):
            laplacian_term = (
                Cm_old[j, i + 1] - 2 * Cm_old[j, i] + Cm_old[j, i - 1]
            ) / dr**2
            ...
            Cm[j, i] = (
                Cm_old[j, i]
                + (dCm_dt_diffusion + G - dCm_dt_trapping + dCm_dt_detrapping) * dt
            )
        ...
        Cm[j, N] = (
            ...
            + (
                dCm_dt_diffusion_in
                - dCm_dt_surface_release_out
                + G
                - dCm_dt_trapping_surface
                + dCm_dt_detrapping_surface
            )
            * dt
        )
        Ct[j, N] = (
            Ct_old[j, N] + (dCm_dt_trapping_surface - dCm_dt_detrapping_surface) * dt
        )

        # Pore Gas Concentration
        J_pellet[j] = h_pellet * (C_pore_old[j] - C_sparge_old[j])
        C_pore[j] = C_pore_old[j] + (
            dt * (J_grain[j] * A_internal - J_pellet[j] * A_external) / V_pore
        )
    ...
    for j in range(1, Nz):
        ...
        C_sparge[j] = C_sparge_old[j] + dt * (
            source_term + convection_term_in - convection_term_out
        )
    ...
    if step % plot_interval == 0:
        ...
        inventory_grain_inlet = np.sum(
            total_conc_grain * 4 * np.pi * r**2 * dr
        )  # Total Tritium inventory per grain at inlet (atoms)
        for j in range(Nz):
            total_conc_grain = Cm[j, :] + Ct[j, :]
            inventory_grain = np.sum(
                total_conc_grain * 4 * np.pi * r**2 * dr
            )  # Total Tritium inventory per grain (atoms)
            solid_inventory += (
                inventory_grain * N_grains_pellet * N_pellets_plug * decay_constant
            )  # Solid phase inventory (Bq)
            pore_inventory += (
                C_pore[j] * V_pore * N_pellets_plug * decay_constant
            )  # Pore gas inventory (Bq)
            sparge_inventory += (
                C_sparge[j] * V_sparge_plug * decay_constant
            )  # Sparge gas inventory (Bq)
        total_system_inventory = (
            solid_inventory + pore_inventory + sparge_inventory
        )  # Total system inventory (Bq)
        ...
        total_generated = (
            G_rate * min(current_time, t_irr) * V_breeding_total * decay_constant
        )  # Total generated tritium (Bq)
        total_generated_history.append(total_generated)

        # Release Rate
        release_rate = (
            C_sparge[Nz - 1] * Q_sparge * decay_constant
        )  # Release rate at the outlet (bq/s)
        bed_release_rate_history.append(release_rate)
        grain_release_rate = (
            J_grain[0] * fr * A_grain
        )  # Grain release rate (at inlet, j = 0) (atoms/s)
        ...
        for key_time in key_times:
            if abs(current_time - key_time) < key_time_tol:
                C_sparge_profile_history[f"{current_time/3600:.1f} hr"] = (
                    C_sparge.copy()
                )
...
results = {
    ...
    "sparge_gas_axial_profiles": {
        k: v.tolist() for k, v in C_sparge_profile_history.items()
    },
