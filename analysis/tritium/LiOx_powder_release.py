# -----------------------------------------------------------------------------
# Tritium Transport in Lithium Ceramic Powder, with Packed Bed (Plug Flow Model)
#
...
print("==============================================================")
print("Tritium Transport in Lithium Ceramic Powder, with Packed Bed (Plug Flow Model)")
print("==============================================================\n")
...
A_grains_plug = (
    N_grains_plug * A_external
)  # Total External Surface Area of Grains in one plug (cm^2)
...
while current_time < total_sim_time:
    ...
    if step % plot_interval == 0:
        ...
        # Release Rate
        release_rate = (
            C_sparge[Nz - 1] * Q_sparge
        )  # Release rate at the outlet (atoms/s)
        bed_release_rate_history.append(release_rate)
        grain_release_rate = (
            J_grain[0] * A_external
    ...
    max_rel_change = 0
    for arr, arr_old in [(Cm, Cm_old), (Ct, Ct_old), (C_sparge, C_sparge_old)]:
...
cumulative_release = (
    np.cumsum(np.array(bed_release_rate_history) * data_intervals) * decay_constant
)
