...
$$
-D \left( \frac{\partial C_m}{\partial r} \right)_{r=r_{grain}} = J_{grain}
$$
...
* **Surface Node ($i=N$):** The boundary condition is enforced using a "ghost point" method. Substituting the discretized first and second derivatives into the diffusion part of the governing equation yields:
    $$
    \left( \frac{\partial C_m}{\partial t} \right)_{\text{diffusion, surface}} = \frac{2D(C_{N-1}-C_N)}{\Delta r^2} - 2 J_{grain} \left( \frac{1}{\Delta r} + \frac{1}{r_g} \right)
    $$
    To account for sintering, the release term (related to $J_{grain}$) is scaled by the surface area reduction factor, $F_r$:
    $$
    \left( \frac{\partial C_m}{\partial t} \right)_{\text{diffusion, surface}} = \underbrace{\frac{2D(C_{N-1}-C_N)}{\Delta r^2}}_{\text{Diffusion from interior}} - \underbrace{F_r \cdot 2 J_{grain} \left( \frac{1}{\Delta r} + \frac{1}{r_g} \right)}_{\text{Effective release from surface}}
    $$
## 3. Inter-Grain and Pellet Transport
